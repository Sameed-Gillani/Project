...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_student'])) 
   {$name = trim($_POST['name']);
    ...
    if (!empty($name) && !empty($email) && !empty($deptId)) 
        {try 
           {$stmt = $pdo->prepare("INSERT INTO students (name, email, department_id) VALUES (?, ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_student'])) 
   {$studentId = $_POST['student_id'];
    ...
    if (!empty($name) && !empty($email) && !empty($deptId)) 
       {try 
           {$stmt = $pdo->prepare("UPDATE students SET name = ?, email = ?, department_id = ? WHERE id = ?");
            $stmt->execute([$name, $email, $deptId, $studentId]);
